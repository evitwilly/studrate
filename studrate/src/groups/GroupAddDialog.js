...
export default class GroupAddDialog extends React.Component {
	...
	apply() {
		if (this.groupInput.value.length <= 0) {
			this.setState({error: "Пустое название группы"})
		} else {
			axios.post(constants.restData.postGroupAdd, { name: this.groupInput.value }).then(response => {
				this.props.update();
				this.props.dismiss()
			});
		}
	...
	render() {
		...
		if (this.state.error.length > 0) {
			errorView = <p className="core_input_error">{this.state.error}</p>
		}
		...
		return (
			<div className="core_container_dialog">
				<div className="core_dialog">
					...
					<input className="core_dialog_input mb_16" ref={(input) => this.groupInput = input} placeholder="КСК-18" />
					{errorView}
					<div className="core_dialog_apply_button" onClick={() => this.apply()}>Добавить</div>
				</div>
			</div>
		);
	}
