...
export default class StudentAddDialog extends React.Component {
	...
	componentDidMount() {
		axios.get("http://localhost:3434/groups").then((response) => {
	...
	apply() {
		...
		if (fioData.status == "success" && ratingData.status == "success" && priorityError.length <= 0) {
			...
			if (this.props.student != undefined) {
				...
				axios.post("http://localhost:3434/student/update", data).then(response => {
				...
				axios.post("http://localhost:3434/students/add", data).then(response => {
					if (response.data.status == "success") {
						this.props.dismiss();
						this.props.update();
					}
	...
	render() {
		...
		if (this.state.fioData.status == "error") {
			fioError = <p className="student_input_error">{this.state.fioData.message}</p>;
		}

		let ratingError;
		if (this.state.ratingData.status == "error") {
			ratingError = <p className="student_input_error">{this.state.ratingData.message}</p>;
		}

		let priorityError;
		if (this.state.priorityError.length > 0) {
			priorityError = <p className="student_input_error">{this.state.priorityError}</p>;
		}

		const buttonText = this.props.student != undefined ? "Изменить" : "Добавить";

		return (
			<div className="student_container_dialog">
				<div className="student_dialog">
					<h4 className="student_dialog_title">Добавление студента<div className="close_button" onClick={this.props.dismiss}>
						<svg className="close_button_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M310.6 361.4c12.5 12.5 12.5 32.75 0 45.25C304.4 412.9 296.2 416 288 416s-16.38-3.125-22.62-9.375L160 301.3L54.63 406.6C48.38 412.9 40.19 416 32 416S15.63 412.9 9.375 406.6c-12.5-12.5-12.5-32.75 0-45.25l105.4-105.4L9.375 150.6c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0L160 210.8l105.4-105.4c12.5-12.5 32.75-12.5 45.25 0s12.5 32.75 0 45.25l-105.4 105.4L310.6 361.4z"/></svg>
					</div></h4>
					<input className="student_input" 
					...
					{priorityError}
					<div className="add_button noselect" onClick={() => this.apply()}>{buttonText}</div>
				</div>
				{groupDialog}
			</div>
		);
